...
function getProposal(event) {
    ...
    xhr.onload = function () {
        ...
        while (search_box.childNodes.length > 2) {
            search_box.removeChild(search_box.lastChild);
        }

        for (let i = 0; i < this.response.length; ++i) {
            let proposal = document.createElement('div');
            proposal.className = "search-proposal";
            proposal.style.cursor = "pointer";
            proposal.innerHTML = this.response[i];
            proposal.onclick = setQuery;
            search_box.appendChild(proposal);
        }
    };
    xhr.send();
}
...
window.onload = function () {
    ...
    document.querySelector('.search-field').oninput = getProposal;
};